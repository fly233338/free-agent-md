...
- `document.instructions` (the repo's documentation placement policy), `review.path_instructions` (path-scoped review guidance appended to the review prompt), and `disable_project_settings` (the gate-agent project-instruction opt-out) are also trusted-only, regardless of `allow_repo_commands`: a pushed branch must not weaken any of those boundaries, and enabling the commands opt-in must not drop the maintainer's own trusted values. When the opt-out is enabled, only adapters with verified effective suppression may launch. Non-executing fields (`ignore_patterns`, `auto_fix`, `commit`, `intent`, `test`) are still read from the pushed branch.
